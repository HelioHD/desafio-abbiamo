...
const create_timesheetRequest_dto_1 = require("./dto/create-timesheetRequest.dto");
...
let OrderProducerController = class OrderProducerController {
    ...
    async createTimesheetRequest(createOrder, req) {
        try {
            common_1.Logger.debug('Recebida a requisição', 'timesheet-producer', 'create', createOrder);
            common_1.Logger.debug('Recebida a requisição', 'timesheet-producer', 'req', req);
            if (createOrder) {
                const request = await this.orderProducerService.createTimesheetRequest(createOrder);
                return request;
            }
        }
        catch (error) {
...
__decorate([
    ...
    __metadata("design:paramtypes", [create_timesheetRequest_dto_1.CreateTimesheetRequestDto, Object]),
